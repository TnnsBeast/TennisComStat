...
### Clean data sample row

One cleaned observation (after recoding and parsing score context) to show the shape of the dataset that feeds the feature engineering steps.

```{r clean-sample-row, echo=FALSE}
clean_data %>%
  slice(50) %>%
  knitr::kable(table.attr = 'data-no-collapse="true"')
```
...
## Example match stats (one match)

Pull one match (first in the set), compute basic stats for each player, and visualize them side-by-side.

```{r match-stats, message=FALSE, warning=FALSE}
example_match_id <- clean_data$match_id[[1]]
match_df <- clean_data %>% filter(match_id == example_match_id)

if (nrow(match_df) > 0) {
  detect_flag <- function(col, pattern) {
    str_detect(str_to_lower(coalesce(col, "")), pattern)
  }

  match_df <- match_df %>%
    mutate(
      server_name   = if_else(server == "P1", p1_name, p2_name),
      returner_name = if_else(server == "P1", p2_name, p1_name),
      server_won    = winner == server,
      returner_won  = !server_won,
      ace           = detect_flag(x1_2_rally_outcome, "ace") | detect_flag(x6_1_cheap_point, "ace"),
      double_fault  = detect_flag(x1_2_rally_outcome, "double") | detect_flag(x1_2_1_rally_outcome_detailed, "double"),
      net_approach  = detect_flag(x5_4_serve_1_shot_hitting_point, "net")
    )

  players <- unique(c(match_df$p1_name, match_df$p2_name))

  summarize_player <- function(player) {
    tibble(
      player = player,
      aces = sum(match_df$ace & match_df$server_name == player, na.rm = TRUE),
      double_faults = sum(match_df$double_fault & match_df$server_name == player, na.rm = TRUE),
      first_serve_points = sum(match_df$server_name == player & match_df$x3_2_serve_order == "1st serve", na.rm = TRUE),
      first_serve_points_won = sum(match_df$server_name == player & match_df$server_won & match_df$x3_2_serve_order == "1st serve", na.rm = TRUE),
      second_serve_points_won = sum(match_df$server_name == player & match_df$server_won & match_df$x3_2_serve_order == "2nd serve", na.rm = TRUE),
      return_points_won = sum(match_df$returner_name == player & match_df$returner_won, na.rm = TRUE),
      break_points_won = sum(match_df$returner_name == player & match_df$returner_won & match_df$is_break_point == 1, na.rm = TRUE),
      net_approaches = sum(match_df$net_approach & match_df$server_name == player, na.rm = TRUE),
      total_points_won = sum(
        (match_df$server_won & match_df$server_name == player) |
          (match_df$returner_won & match_df$returner_name == player),
        na.rm = TRUE
      )
    )
  }

  match_stats <- map_dfr(players, summarize_player)
  ...
  match_stats_long %>%
    ...
    coord_flip() +
    labs(
      title = "Example match stats (one match)",
      subtitle = paste("Match ID:", example_match_id),
      x = NULL,
      y = "Count / points",
      fill = "Player"
    ) +
  ...
  message("No match data available to summarize.")
}
```
...
# Show the buckets that shifted accuracy the most (lowest) and least (highest) — top/bottom 3
lofo_extremes <- bind_rows(
  slice_min(lofo_results, accuracy, n = 3, with_ties = TRUE),
  slice_max(lofo_results, accuracy, n = 3, with_ties = TRUE)
) %>%
  distinct()

lofo_extremes %>%
  ggplot(aes(x = reorder(bucket, accuracy), y = accuracy, fill = accuracy == min(accuracy))) +
  geom_col() +
  scale_fill_manual(
    values = c("TRUE" = "indianred3", "FALSE" = "seagreen3"),
    labels = c("Most change", "Least change"),
    guide = guide_legend(title = NULL)
  ) +
  coord_flip() +
  labs(
    title = "Top/bottom LOFO buckets",
    x = "Bucket removed",
    y = "Accuracy (held-out)"
  ) +
  theme_minimal(base_size = 13) +
